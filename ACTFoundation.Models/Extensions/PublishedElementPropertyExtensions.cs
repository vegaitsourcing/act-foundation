...
namespace ACTFoundation.Models.Extensions
{
	public static class PublishedElementPropertyExtensions
	{
		...
		/// </summary>
		/// <remarks>
		/// This method is exactly the same as Umbraco's own <see cref="Umbraco.Web.PublishedElementExtensions.Value{T}(IPublishedElement,string,string,string,Fallback,T)"/>,
		...
		public static T GetPropertyValue<T>(this IPublishedElement source, [CallerMemberName] string propertyName = null)
			=> source.Value<T>(propertyName);
		...
		public static T GetPropertyWithDefaultValue<T>(this IPublishedElement source, T defaultValue, [CallerMemberName] string propertyName = null)
			=> source.HasValue(propertyName) ? source.Value<T>(propertyName) : defaultValue;
