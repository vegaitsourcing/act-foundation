...
using ACTFoundation.Core.Controllers.RenderMvc;
using ACTFoundation.Core.Extensions;
using ACTFoundation.Core.Handlers;
...
namespace ACTFoundation.Core
{
	public class RouteConfig
	{
		...
		private static string GetError404RouteUrl()
		{
			...
			string xml = WebConfigurationManager.OpenWebConfiguration("/").GetSection("system.webServer").SectionInformation.GetRawXml();
			string url = (XDocument.Parse(xml).Root.Element("httpErrors")?.Elements("error"))?.FirstOrDefault(e => e.Attribute("statusCode")?.Value == "404")?.Attribute("path")?.Value;
