...
namespace ACTFoundation.Search.Services.Implementation
{
	public class SearchService : DisposableObjectSlim, ISearchService
	{
		...
		public SearchService(IExamineManager examineManager, IUmbracoContextAccessor umbracoContextAccessor, string indexName, string[] searchFields)
		{
		...
		public SearchResultsPerPage Search(string query, int page, int itemsPerPage, Dictionary<string, string> additionalSearchParameters)
		{
			return Search(query, page, itemsPerPage, IndexTypes.Content, BooleanOperation.And, additionalSearchParameters);
		}
		...
		public SearchResultsPerPage Search(string query, int page, int itemsPerPage, string searchType, BooleanOperation searchOperation)
		{
			...
			Dictionary<string, string> additionalSearchParameters)
		{
			...
			int maxResults = itemsPerPage * page;
		...
		private string BuildQuery(string query, string searchType, BooleanOperation searchOperation, Dictionary<string, string> additionalSearchParameters)
		{
			...
			if (additionalSearchParameters == null || !additionalSearchParameters.Any()) return luceneQuery.Query.ToString();

			LuceneSearchQuery additionalLuceneQuery = CreateLuceneSearchQuery(searchType, searchOperation);
